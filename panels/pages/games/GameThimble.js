...
export default class GameThimble extends React.Component {
   ...
   renderBalanceBlock = () => {
            ...
            return (<div className='buttonsBlock game'>
                <FormLayout className='form'>
                    <FormStatus className='winStatusDef'
                                    header="Где спрятан приз?"
                                    mode={"default"}>
                            <div
                                className='verticalText'>Выберите один из трёх стаканчиков</div>
                            <div className='coef'>x2.7</div>
                        </FormStatus>
                </FormLayout>
            </div>);
        }
                    ...
                    <FormStatus className='winStatus'
                        ...
                        {this.state.gameData.win > 0 ?
                            <div style={{paddingLeft: '5px'}}
                                 className='verticalText'> (x2.7)</div> : null}
                    </FormStatus>
    ...
    renderTable = () => {
        ...
        if (this.state.gameData.state === 1) {
            ...
            let winThimble = -1;
            if (this.state.gameData.state === 3){
                winThimble = this.state.gameData.res;
            }

            for (let i = 0; i < 3; i++) {
                let state = this.state.gameData.open[i];
                if (winThimble >= 0){
                    if (winThimble === i){
                        state = 1;
                    } else {
                ...
                thimbles.push(<div className={'thimble state' + state} onClick={() => this.openThimble(i)}>
                    <img src={ThimbleGreyImage} className='tgImage' alt='t'/>
                    <img src={ThimbleImage} className='tImage' alt='t'/>
                    <img src={CoinsImage} className='coins' alt='t' />
                </div>);
            }
    ...
    renderUsers = () => {
        ...
        for (let i = 0; i < this.state.gameData.history.length; i++) {
            ...
            let color = this.state.gameData.history[i][4];
            let nameServer = this.state.gameData.history[i][5];
            let crown = this.state.gameData.history[i][6];
            let name = '@' + uid;
            let photo = null;
    ...
    render() {
        return (
            <View className='gameThimble' activePanel={this.state.activePanel} popout={this.state.popout} header={true}>
                <Panel id='game'>
                    <PanelHeader>
                ...
                this.setState({
                    snackbar: <Snackbar style={{zIndex: 1000}}
                                        ...
                                        before={<Avatar size={36}
                                                        style={{background: '#ff1744'}}><Icon28FireOutline width={24} height={24}
                                                                                                fill='#ffffff'/></Avatar>}
                    >Вы угадали стаканчик с первой попытки 3 раза подряд! Ваш приз: <b>{SystemFunctions.formatNumber(msg.sum)}</b> VKC!</Snackbar>
                });
                break;
        }
    }

...
import IconGold from '../../../img/icon_gw_gold';
import IconTnt from '../../../img/icon_gw_tnt';
...
export default class GameGoldWest extends React.Component {
            ...
            if (this.state.gameData.state === 1) {
                ...
                for (let i = 0; i < 5; i++) {
                    if (i < this.state.bombs) {
                        bombsInd.push(<div className={'ic on'}><IconTnt width={48} height={48}/></div>);
                    } else {
                        bombsInd.push(<div className={'ic off'}><IconGold width={48} height={48}/></div>);
                    }
                }
                return (<div className='table'>
                    <div className='info'>
                        <div className={'block x2 ' + (this.state.bombs === 1 ? 'selected' : '')} onClick={() => this.setState({bombs: 1})}>
                            <div className='header'>1 бомба</div>
                            <div className='x'>X2</div>
                        </div>
                        <div className={'block x3 ' + (this.state.bombs === 2 ? 'selected' : '')} onClick={() => this.setState({bombs: 2})}>
                            <div className='header'>2 бомбы</div>
                            <div className='x'>X3</div>
                        </div>
                    </div>
                ...
                for (let i = 0; i < 10; i++) {
                    let lr = [];
                    let inLine = 2;
                    if (this.state.gameData.bombs === 2){
                        inLine = 3;
                    }
                        ...
                        if (this.state.gameData.state === 3) {
                            ...
                            if (this.state.gameData.field[i][j] === false) {
                                lr.push(<div className={'item w ' + cl}>
                                    <div className='ic'><IconGold width={48} height={48}/></div>
                                </div>);
                            } else {
                                lr.push(<div className={'item b ' + cl}>
                                    <div className='ic'><IconTnt width={48} height={48}/></div>
                                </div>);
                            }
                        } else {
                            if (this.state.gameData.open[i] === -1) {
    ...
    render() {
        return (
            <View className='gameGoldWest' activePanel={this.state.activePanel} popout={this.state.popout} header={true}>
                <Panel id='game'>
                    <PanelHeader>
    ...
    genCoefs = (mode) => {
        ...
        for (let i = 0; i < 10; i++) {
            c *= 1 - bombs / inLine;
            let cn = Math.floor(0.95 / c * 100) / 100;
            r.push(cn);
        }
        return r;
    }
